      ...
      Vue.set(this.state, 'updatingLinks', {});
		...
		getNodeDimensions(){
			...
			const dimensions = mapValues(nodes, (node, id) => {
				...
				const { width, height, bottom, right, top } = el.getBoundingClientRect()
				return { width, height, bottom, right, top, styleTop: parseFloat(el.style.top.substring(0, el.style.top.length - 3)), styleBottom: parseFloat(el.style.left.substring(0, el.style.left.length - 3)) }
			})
			return dimensions
		},
		...
		recenter: function () {
			...
			const xFactor = divstorm[0].clientWidth / maxRight;
			const yFactor =divstorm[0].clientHeight / maxBottom;
			const zoomFac = xFactor < yFactor ? xFactor : yFactor;
